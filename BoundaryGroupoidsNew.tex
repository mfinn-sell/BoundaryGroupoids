...
Groupoids play an prominent role in topology and geometry, particularly influencing coarse geometry through the work of \cite{}. We recall the definition of a groupoid in this context and give examples to illustrate the imporant features required for the later parts of the paper.
...
\begin{example}Let $X$ be a coarse space with uniformly locally finite, weakly connected coarse structure $\mathcal{E}$, such as a uniformly discrete metric space $X$ with its metric coarse structure. Define $G(X):=\cup_{E\in \mathcal{E}}\overline{E}.$ Then $G(X)$ is a locally compact, \'etale groupoid with the induced product, inverse and topology from $\beta X \times \beta X$.
\end{example}
By considering the metric coarse structure we can reduce this to considering only generators:
\begin{equation*}
