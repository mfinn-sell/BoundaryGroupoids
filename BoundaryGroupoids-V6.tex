...
\end{theorem}
\begin{proof}
The first part follows from the definition of $\pi(q)$; the quotient map $h$ kills\setlength{\parindent}{0pt}
\setlength{\parskip}{1ex} all operators that are ghostly in the direction $i \rightarrow \infty$ and $\pi(q)$ is such an operator.
...
\end{eqnarray*}
The ideal $C^{*}_{r}(G(Y)|_{U})= \bigoplus_{i}M_{\vert X_{i}\vert} \otimes C(\partial\beta Y)$ maps to the ideal $\bigoplus_{i}C^{*}_{r}(G(X_{i} \times \mathbb{N}))$ under the map $\phi$. So, we can define a tracelike map, in analogy to \cite[Section 6]{explg1}, by composing with the quotient map $\tau$ onto $\frac{\prod_{i}C^{*}_{r}(G(X_{i} \times \mathbb{N}))}{\bigoplus_{i}C^{*}_{r}(G(X_{i} \times \mathbb{N}))}$. This gives us a map at the level of K-theory:
\begin{equation*}
