...
This groupoid plays an important role the groupoid formulation of the coarse Baum-Connes conjecture and the most general consequences of that conjecture. We recall some results concerning this groupoid from the literature:
\begin{theorem}
...
\end{equation*}
Now we can consider projection $p$ onto the kernel of $\Delta$. For an expander $X$ we have $spectrum(\Delta) \subseteq \lbrace 0 \rbrace \cup [c,1]$ for some $c>0$, and so by an application of the functional calculus we can conclude that $p \in C^{*}X$. As $\Delta$ breaks up as a product we observe that its $Ker(\Delta)=\oplus_{i} Ker(\Delta_{i})$ and so the projection $p$ decomposes as a product:
\begin{equation*}
p=\prod_{i}p^{(i)}.
\end{equation*}
